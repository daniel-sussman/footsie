class GamesController < ApplicationController
  ...
  def create
    ...
    red_team = Team.create(name: red_team_name, game_id: @game.id)
    PlayerTeam.create(player_id: current_player.id, team_id: red_team.id)
    Team.create(name: blue_team_name, game_id: @game.id)
    if @game.save
  ...
  def game_params
    params.require(:game).permit(:name, :description, :red_team_name, :blue_team_name, :price, :gender, :team_size, :pitch_identifier, :pitch_type, :indoor, :address, :starting_date, :ending_date, :recurring_rule, :photo)
  end
